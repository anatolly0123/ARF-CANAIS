...
export function useStore(user: User | null) {
  ...
  const [todayMessage, setTodayMessage] = useState<string>(() => {
    const saved = localStorage.getItem('arf_today_message');
    const defaultMsg = 'Olá *{nome}*! 👋\n\nPassando para avisar que seu acesso vence *HOJE*! ⏳\n\nO valor para renovação é de *{valor}*.\n\nPodemos confirmar sua renovação para garantir que seu sinal não seja cortado? 😊';
    return saved || defaultMsg;
  });
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        if (customersData) {
          const localCustomersRaw = localStorage.getItem('arf_customers');
          const localCustomers: any[] = localCustomersRaw ? JSON.parse(localCustomersRaw) : [];
          const localCountryMap = new Map(localCustomers.map(lc => [lc.id, lc.country]));
  ...
  const deleteServer = async (id: string) => {
    ...
    if (linkedCustomers.length > 0) {
      alert(`Não é possível excluir este servidor pois há ${linkedCustomers.length} cliente(s) vinculado(s) a ele. Transfira ou exclua os clientes antes de excluir o servidor.`);
      return;
    }
    ...
    if (user) {
      ...
      if (error) {
        ...
        alert('Erro ao excluir servidor: ' + (error.message || 'Ocorreu um erro inesperado.'));
        return;
      }
    }
  ...
  useEffect(() => {
    localStorage.setItem('arf_today_message', todayMessage);
  }, [todayMessage]);
    ...
    try {
      const dataToSync = {
        ...
        settings: {
          ...
          todayMessage: overrideData?.settings?.todayMessage ?? todayMessage,
